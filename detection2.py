...
for j in range(img.shape[1]):# 广搜
    for i in range(img.shape[0]):
        if (visit[i][j]==0 and img[i, j] < 30):
            ...
            cal_y0 = sess.run(prob, feed_dict={x: mnist_array})
            cal_re0 = sess.run(result, feed_dict={x: mnist_array})
            ...
            mnist_array = numpy.reshape(mnist_img, (1, 784))
            mnist_array = mnist_array.astype(numpy.float32)
            cal_y1 = sess.run(prob, feed_dict={x: mnist_array})
            cal_re1 = sess.run(result, feed_dict={x: mnist_array})
